...
import { Settings, Shield, Link as LinkIcon, Sliders, HardDrive, Database, Settings2, Activity, MessageSquare, Heart } from 'lucide-react';
      ...
      const fetchLatestPosts = async () => {
         try {
            ...
            if (res.ok) {
               ...
               // Take only top 2 latest posts
               setLatestPosts(data.slice(0, 2));
            }
         } catch (err) {
            console.error("Failed to fetch ecosystem pulse", err);
         }
      };
      fetchLatestPosts();
   }, []);
         ...
         <section className="pulse-section">
            <div className="pulse-header">
               <div className="pulse-title-group">
                  ...
                  <h3>Live Ecosystem Pulse</h3>
               </div>
               <span className="pulse-status">SYNCING REAL-TIME DATA</span>
            </div>
            
            <div className="pulse-grid-container">
               {latestPosts.length === 0 ? (
                  <div className="pulse-empty">Awaiting community signals...</div>
               ) : (
                  latestPosts.map(post => (
                     <div key={post.id} className="pulse-item" onClick={() => navigate('/community')}>
                        <div className="pulse-item-top">
                           <span className="pulse-author">{post.authorName}</span>
                           <span className="pulse-handle">{post.authorHandle}</span>
                        </div>
                        <p className="pulse-text">{post.text.substring(0, 80)}{post.text.length > 80 ? '...' : ''}</p>
                        <div className="pulse-stats">
                           <span className="pulse-metric"><Heart size={12} fill="#ef4444" color="#ef4444" /> {post.likes?.length || 0}</span>
                           <span className="pulse-metric"><MessageSquare size={12} /> {post.comments || 0}</span>
                        </div>
                     </div>
                  ))
               )}
