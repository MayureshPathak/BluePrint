...
function DepartmentPage() {
  ...
  const handleDeleteNote = async (noteId) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/notes/${noteId}?email=${encodeURIComponent(user.email)}`, {
        method: 'DELETE'
      });
      if (res.ok) {
  ...
  const fetchNotes = async () => {
    try {
      ...
      const url = `${API_BASE}/api/notes?year=${encodeURIComponent(activeYear)}&departmentId=${activeDeptId}`;
              ...
              <div key={note.id} className="note-card">
                ...
                <div className="note-footer">
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    ...
                    <span className="note-action" style={{ cursor: 'pointer' }} onClick={() => requireAuthAction(() => window.open(`${API_BASE}${note.filePath}`, '_blank'))}>
                      <Download size={18} />
                    </span>
                  </div>
                </div>
