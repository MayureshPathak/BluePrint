...
function CommunityPage() {
  ...
  const fetchNotifs = async () => {
     ...
     try {
        const res = await fetch(`${API_BASE}/api/auth/user/${user.email}/notifications`);
        if (res.ok) {
           const data = await res.json();
  ...
  const toggleNotifMenu = async () => {
     ...
     if (nextState && notifications.filter(n => !n.read).length > 0) {
        try {
           await fetch(`${API_BASE}/api/auth/user/${user.email}/notifications/read`, { method: 'POST' });
  ...
  const handleDeletePost = async (postId) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/posts/${postId}?email=${encodeURIComponent(user.email)}`, {
        method: 'DELETE'
      });
      if (res.ok) {
                        ...
                        <div className="post-content-area">
                           ...
                           {post.imageFile && (
                              <div style={{ width: '100%', borderRadius: '0.5rem', overflow: 'hidden', marginBottom: '0.75rem', border: '1px solid #f1f5f9' }}>
                                 <img src={post.imageFile.startsWith('http') ? post.imageFile : `${API_BASE}${post.imageFile}`} alt="Post attachment" style={{ width: '100%', display: 'block', maxHeight: '400px', objectFit: 'contain', backgroundColor: '#f8fafc' }} />
                              </div>
                           )}
